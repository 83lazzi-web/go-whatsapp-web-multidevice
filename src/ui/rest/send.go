...
func InitRestSend(app fiber.Router, service domainSend.ISendUsecase) Send {
	...
	app.Post("/send/verify-user", rest.VerifyUser)
	return rest
}
...
func (controller *Send) VerifyUser(c *fiber.Ctx) error {
	var request domainSend.MessageRequest
	err := c.BodyParser(&request)
	utils.PanicIfNeeded(err)
	...
	response, err := controller.Service.VerifyUserValid(c.UserContext(), request)
	utils.PanicIfNeeded(err)
